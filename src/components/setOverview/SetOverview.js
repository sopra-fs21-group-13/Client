...
function SetOverview(props) {
  ...
  const [addButtonDisabled, setButton] = useState(false);
  ...
  useEffect(() => {
    ...
    var setId = location.clickedSet.setId;
    ...
    api
      ...
      .then((response) => {
        ...
        if(response.data.memberIds.includes(localStorage.getItem("userId"))){
            setButton(true);
        }else{
            setButton(false);
        }
  ...
  //add a set to user dashboard.
  function addToDashboard(set) {
    api
      .put("/sets/" + localStorage.getItem("userId") + "/" + set.setId)
      .then((response) => {
        console.log("added set " + set.setId + " to users dashboard");
      })
      .catch((e) => {
        alert(
          `Something went wrong while adding set to dashboard: \n${handleError(
            e
          )}`
        );
      });
    ...
    <div>
      ...
      <div id="screen">
        <div class="back-button-container_overview">
          <button class="back-button" onClick={history.goBack}>
              ...
              <form class="editForm">
                ...
                <div id="basic_info">
                  <div className="infoBlock">
                    ...
                    {
                        //checks first if set is fetched
                      }
                      {!set || localStorage.getItem("userId") == set.userId ? (
                        <div>
                        ...
                        <Button
                          ...
                          disabled={
                            addButtonDisabled ||
                            localStorage.getItem("userId") == set.userId ||
                      ...
                      {cards.map((quiz) => (
                        <div class="qna">
                          ...
                          <div class="qna_card">
                            ...
                            <div class="a_content">
                              <div className="question_text">{quiz.answer}</div>
                            </div>
                          </div>
                        </div>
                      ))}
